...
SFT searches over student policies to optimize. This is typically framed as minimizing the KL-divergence from the teacher to the student, which is equivalent to maximizing the log-likelihood of the teacher's data:

$$
\begin{align}
\argmin_{p} KL[p^*(x) || p(x)]
&= \argmin_p E_{p^*(x)}[\log p^*(x) - \log p(x)] \\
&= \argmax_p E_{p^*(x)}[ \log p(x)].
\end{align}
$$
...
&= \argmax_p E_{p^*(x)}[\frac{p(x)}{p^*(x)} r(x)].
\end{align}
$$
