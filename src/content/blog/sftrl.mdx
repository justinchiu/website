...
This are puzzling. Why would we ever want to train on incorrect examples?
...
The teacher gives demonstration trajectories $\{x_i\}$ which serve as examples to teach the student.
SFT searches over student policies to optimize

$$
\begin{align}
\argmin_{p} KL[p^*(x) || p(x)]
&= \argmin_p E_{p^*(x)}[\log p^*(x) - \log p(x)] \\
&= -H[p^*(x)] - \argmax_p E_{p^*(x)}[ \log p(x)].
\end{align}
$$

<details class="highlights">
<summary><b>Tangent 1: KL vs Cross-Entropy</b></summary>
<div style="padding-top: 10px;">
The differences between equation (1) and (2) are interesting.
We used to normally only see (2) in traditional ML, where we were learning from Human teachers.
We usually do not have access to calibrated probabilities of human demonstrations.
But nowadays, we have actually have a lot of teacher models in the form of open weight LLMs, letting us optimize (1).
...
This looks similar to off-policy policy gradient, where you optimize
$$
\begin{align}
\argmax_p E_{p(x)}[r(x)]
&= \argmax_p \int_x p(x)r(x)\\
&= \argmax_p \int_x p(x) \frac{p^*(x)}{p^*(x)} r(x)\\
&= \argmax_p E_{p^*(x)} \frac{1}{p^*(x)} p(x) r(x).
\end{align}
$$
Let's examine each of the three terms in equation (5), and contrast them with the rightmost term of equation (2).
1. Importance weight denominator $1/p^*(x)$: Since the trajectories are sampled from $p^*(x)$ it's reasonable to assume that the samples are high probability under $p^*$ and this can pretty safely be ignored.
2. Importance weight numerator $p(x)$: This is $p(x)$ rather than $\log p(x)$. Since they are monotonic transformations of another, this may not affect learning too much.
3. Reward $r(x)$: In the SFT setting, the teacher trajectories are assumed to have reward $r(x)=1$.
