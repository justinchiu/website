...
We can greatly simplify the derivation of the exact gradient estimator by applying the log-derivative trick at equation 5 instead of the product rule (exercise for the reader, see footnote for a big hint[^hint]). This is mostly clear in hindsight, and the tool of brute-force derivation is more useful for a first approach. Let's not pretend things are elegant on the first attempt.
</details>
