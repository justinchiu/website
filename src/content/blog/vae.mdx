...
description: 'Latent variable models require RL'
pubDate: 'Aug 4 2025'
draft: true
---
...
This post is slightly longer than usual and aims to help you reason through the problem solving process of variational inference.
Maybe this is basically an attempt at doing some data annotation for reasoning LLMs.
...
and training it on a dataset consisting of a single observations $y$ (WLOG [^WLOG]).
...
We can greatly simplify the derivation of the exact gradient estimator by applying the log-derivative trick at equation 5 instead of the product rule (exercise for the reader[^hint]). This is mostly clear in hindsight, and the tool of brute-force derivation is more useful for a first approach. Let's not pretend things are elegant on the first attempt.
</details>
