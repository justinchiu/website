...
The most convenient approach to optimizing the evidence is to compute it exactly
and compute gradients using automatic differentiation.
Howeer, computing this sum is intractable if the space of $z$ is really large.
...
that are correlated with the evidence and also admit computationally cheap gradient estimators
for optimization.

We will explore a series of surrogate objectives, deriving:
1. The relationship between the surrogate objective and the evidence
2. Gradient estimators to optimize each surrogate objective
