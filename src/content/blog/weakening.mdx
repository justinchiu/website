...
If we discard the sampled routing path and try to train only on the marginal mixture over experts, the natural approximations give rise to a **nested Monte Carlo estimator** in the sense of Rainforth et al. (2019)[^rainforth2019], which has provably poor statistical properties.
...
## 3. What routing replay actually does
...
## 4. Why routing replay is (practically) necessary
...
- **Without routing replay**  
  ...
  - When the log-sum is approximated, the resulting estimator is *nested* (see Section 4.1), leading to biased and statistically inefficient gradient estimates for a fixed compute budget.
...
### 4.1 Variance and nested Monte Carlo
