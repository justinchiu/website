...
If this is your first time seeing the ELBo, I'll briefly introduce it along the way.
...
1. The rewards are log-scaled in this derivation, while they are not in STaR.
2. Pulling out the expectation over $y$ results in an even looser lower bound on the ELBo, due to another application of
Jensen's inequality.
...
### ELBo (rationalizer)
When employing the rationalizer $q(r \mid x,y)$ that sees answers before producing rationales, 
we the objective should be the exact ELBo presented earlier,
$$
E_{q(r\mid x,y)}[\log p(y \mid x, r)] - KL[q(r \mid x,y) || p(r \mid x)].
$$
Is this what STaR optimizes?
The answer is yes, with the same caveats as the previous approach.
Namely, the expectation wrt $y$ is pulled out.
...
for example introducing a rationale editor $q(r \mid x, r_{\text{old}}, y)$.
